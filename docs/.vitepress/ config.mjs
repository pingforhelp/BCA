// .vitepress/config.js
import { defineConfig } from "bca" // change to "vitepress" if needed
  ...
  withSidebar(
    defineConfig({
      ...
      themeConfig: {
        ...
        socialLinks: [{ icon: "github", link: "https://github.com/pingforhelp/BCA" }],
      },
          ...
          md.core.ruler.after("inline", "obsidian-callouts", (state) => {
            state.tokens.forEach((token, i) => {
              if (
                ...
                const match = state.tokens[i + 1].content.match(/\[!([a-zA-Z0-9_-]+)\](.*)?/)
                if (match) {
      ...
      vite: {
        plugins: [
          vitepressPythonEditor({
            assetsDir: "docs/.vitepress/dist/assets",
          }),
        ],
      },
    }),
